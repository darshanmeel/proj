...
class NN_single_hidden_layer:
    ...
    def backprop(self,loss):
        ...
        dcache4 = self.nn['dcache4']
        
        dx1 = sigm_backward(loss,dcache4)

        dx2,dw2,db2 = fully_connected_bkwd(dx1,dcache3)
    ...
    def fit(self,train_data,train_class,test_data = None ,test_class= None):
        ...
        for epoch in range(epochs):
            ...
            if test_data is None:
                ...
                dout4  = self.fwd(train_data)         
                train_pred = np.argmax(dout4, axis=1)
                err = sum(train_pred == train_class)
                err = 1.0 - 1.0*err/train_data.shape[0]
                train_err.append(err)
                
                dout4  = self.fwd(test_data)         
                test_pred = np.argmax(dout4, axis=1)
                err = sum(test_pred == test_class)
                err = 1.0 - 1.0*err/test_data.shape[0]
...
if __name__ == '__main__':
    ...
    train_class = np.array([0,1,1,0]).reshape(4,1)
    test_data = train_data
    test_class = train_class
    n_input = train_data.shape[1]
    n_hidden = 2
    n_out = 1
    nn1 = NN_single_hidden_layer(n_hidden,n_input,n_out,epochs = 500,batchsize=1,learning_rate=0.95,loss_fnc=rms_reg)
